...
#include "headers/StringInput.h"

StringInput::StringInput() : text(0), font(0), textColor(0){
	//initialize the string
  setInput("");
	//set font
	TTF_Font* temp = TTF_OpenFont("lib/fonts/Therfont.ttf", 44);
  setFont(temp);
...
void StringInput::handleInput(SDL_Event& gEvent, const unsigned int maxLength){
  ...
  if(gEvent.type == SDL_KEYDOWN){
    ...
    if(temp.length() <= maxLength){
      ...
      if((gEvent.key.keysym.unicode >= static_cast<Uint16>('0')) && (gEvent.key.keysym.unicode <= static_cast<Uint16>('9'))){
        //append the character
        temp += static_cast<char>(gEvent.key.keysym.unicode);
      }else if((gEvent.key.keysym.unicode >= static_cast<Uint16>('A')) && (gEvent.key.keysym.unicode <= static_cast<Uint16>('Z'))){ // if the key is an uppercase letter
        //append the character
        temp += static_cast<char>(gEvent.key.keysym.unicode);
      }else if((gEvent.key.keysym.unicode >= static_cast<Uint16>('a')) && (gEvent.key.keysym.unicode <= static_cast<Uint16>('z'))){ // if the key is a lowercase letter
        //append the character
        temp += static_cast<char>(gEvent.key.keysym.unicode);
      }
    }
    ...
    if(getInput() != temp){
		  setInput(temp);                                                                    // set new input
      SDL_Surface* newInput;                                                             // temporary surface for new input surface
      newInput = TTF_RenderText_Solid(getFont(), getInput().c_str(), *(getFontColor())); // render a new text surface
			setMessageSurface(newInput);                                                       // set new text
    }
  }
}
//*****************************************************
